            ...
            <section>
                <div class="article-detail" data-aos="fade-up" data-aos-anchor-placement="top-bottom" data-aos-delay="150" data-aos-duration="1000">
                    ...
                    <div class="article-txt" data-aos="fade-up" data-aos-anchor-placement="top-bottom" data-aos-delay="150" data-aos-duration="1000">
                        ...
                        <p style="color: #999999;">關於無法將狀態加入收藏或書籤的問題，HTML5 之前的一種方式是使用 URL 片斷識別碼（通常被稱為錨點，即 URL 中 # 後面的部分）來保持追蹤，允許使用者回到指定的某個應用程式狀態。(許多瀏覽器允許JavaScript動態更新錨點，這使得Ajax應用程式能夠在更新顯示內容的同時更新錨點。) HTML5 以後可以直接操作瀏覽歷史，並以字串形式儲存網頁狀態，將網頁加入網頁收藏夾或書籤時狀態會被隱形地保留。上述兩個方法也可以同時解決無法後退的問題。</p>
                        <p style="color: red;">進行Ajax開發時，網路延遲 —— 即使用者發出請求到伺服器發出回應之間的間隔 —— 需要慎重考慮。如果不給予使用者明確的回應，沒有恰當的預讀資料，或者對 XMLHttpRequest 的不恰當處理，都會使使用者感到厭煩。通常的解決方案是，使用一個視覺化的組件來告訴使用者系統正在進行後台操作並且正在讀取資料和內容。</p>
                    </div>
                </div>
            </section>
